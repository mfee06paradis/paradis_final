...
const sql_findById = `SELECT * FROM paradisdb.order where orderId = ?`;
const sql_findByMemberId = `SELECT * FROM paradisdb.order where memberId = ?`;

const sql_findUnpaidOrderIdByMemberId = `SELECT MAX(orderId) AS unpaidOrderId FROM paradisdb.order WHERE orderState = 2 AND memberId = ?`;

//7
const sql_addCartsToOrder = `INSERT INTO paradisdb.order 
